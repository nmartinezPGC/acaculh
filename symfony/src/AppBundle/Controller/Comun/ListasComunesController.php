...
namespace AppBundle\Controller\Comun;
...
class ListasComunesController extends Controller {
     ...
     * Funcion: FND00001
     */
    public function GeneroAllListAction(Request $request) {
        ...
        // Query para Obtener todos los Generos de la Tabla: TblGenero
        $generoAllList = $em->getRepository("BackendBundle:TblGenero")->findAll();

        // Condicion de la Busqueda
        if (count($generoAllList) >= 1) {
            $data = array(
                "status" => "success",
                "code" => 200,
                "data" => $generoAllList
            ...
            $data = array(
                "status" => "error",
                "code" => 400,
                "msg" => "No existe Datos en la Tabla de Genero !!"
            );
        }
...
//FIN | FND00001
    ...
    /**
     * @Route("/profesiones-all-list", name="profesiones-all-list")
     * Creacion del Controlador: Profesiones
     * @author Nahum Martinez <nmartinez.salgado@yahoo.com>
     * @since 1.0
     * Funcion: FND00002
     */
    public function ProfesionesAllListAction(Request $request) {
        date_default_timezone_set('America/Tegucigalpa');
        //Instanciamos el Servicio Helpers y Jwt
        $helpers = $this->get("app.helpers");
        ...
        // Query para Obtener todos las Profesiones de la Tabla: TblProfesion
        $profesionesAllList = $em->getRepository("BackendBundle:TblProfesion")->findBy(
                array(
                    "activo" => true
        ));

        // Condicion de la Busqueda
        if (count($profesionesAllList) >= 1) {
            $data = array(
                "status" => "success",
                "code" => 200,
                "msg" => "Se han encontrado el listado de todas la Profesiones registradas.",
                "data" => $profesionesAllList
            );
        } else {
            $data = array(
                "status" => "error",
                "code" => 400,
                "msg" => "No existe Datos en la Tabla de Profesiones !!"
            );
        }
        //Retorno de la Funcion ************************************************
        return $helpers->parserJson($data);
...
//FIN | FND00002
    ...
    /**
     * @Route("/tipobeca-all-list", name="tipobeca-all-list")
     * Creacion del Controlador: Tipo Becas
     * @author Nahum Martinez <nmartinez.salgado@yahoo.com>
     * @since 1.0
     * Funcion: FND00003
     */
    public function TipoBecaAllListAction(Request $request) {
        date_default_timezone_set('America/Tegucigalpa');
        //Instanciamos el Servicio Helpers y Jwt
        $helpers = $this->get("app.helpers");
        ...
        // Query para Obtener todos los Tipos de Becas de la Tabla: TblTipoBeca
        $tipoBecaAllList = $em->getRepository("BackendBundle:TblTipoBeca")->findAll();

        // Condicion de la Busqueda
        if (count($tipoBecaAllList) >= 1) {
            $data = array(
                "status" => "success",
                "code" => 200,
                "msg" => "Se han encontrado el listado de todas los Tipos de Becas registrados.",
                "data" => $tipoBecaAllList
            );
        } else {
            $data = array(
                "status" => "error",
                "code" => 400,
                "msg" => "No existe Datos en la Tabla de Tipo de Becas!!"
            );
        }
        //Retorno de la Funcion ************************************************
        return $helpers->parserJson($data);
...
// FIN | FND00001
    ...
    public function estadosUsuarioListAction(Request $request) {
        ...
        // Query para Obtener todos los Estados de la Tabla: TblEstado
        $estados = $em->getRepository("BackendBundle:TblEstado")->findBy(
                array(
                    "idGrupo" => 1
        ));
