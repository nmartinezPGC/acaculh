...
@Injectable()
export class EvaluacionPlatoService {
  ...
  * Fecha: 04-01-2019
  * Descripcion: Metodo Ajax, para Invocar el servicio
  * a la API ( /new-evaluacion-plato ).
  * Objetivo: Agregar nueva Eval Cortes
  *****************************************************/
  registerEvaluacionPlato(user_to_register, token) {
    const json = JSON.stringify(user_to_register);
    const params = 'json=' + json + '&authorization=' + token;
    // console.log(json);
    const headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });

    return this._http.post(this.url + '/evaluaciones/new-evaluacion-plato', params, { headers: headers }).map(res => res.json());
  }
  ...
  *****************************************************/
  getEvaluacionPlatoByAlumno(alumno_to_find, token) {
    const json = JSON.stringify(alumno_to_find);
    const params = 'json=' + json + '&authorization=' + token;
    // console.log(json);
    const headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });

    return this._http.post(this.url + '/evaluaciones/list-evaluacion-plato', params, { headers: headers }).map(res => res.json());
  }
