...
@Injectable()
export class ExamenPracticoService {
  ...
  public postFileImagen(token, imagenParaSubir: File, codAlumno: string) {
    ...
    formData.append('cod_examen', codAlumno);
    formData.append('authorization', token);
    // console.log('before hist the service' + JSON.stringify(formData));

    return this._http.post(this.url + '/evaluaciones/upload-examen-practico', formData).map(res => res.json());
  }
  ...
  *****************************************************/
  registerEvaluacionNotaTeorica(token, user_to_register) {
    ...
    return this._http.post(this.url + '/evaluaciones/new-examen-practico', params, { headers: headers }).map(res => res.json());
  }
