...
@Injectable()
export class EvaluacionCortesPrecisionService {
  ...
  * Fecha: 04-01-2019
  * Descripcion: Metodo Ajax, para Invocar el servicio
  * a la API ( /new-evaluacion-cortes-precision ).
  * Objetivo: Agregar nueva Eval Cortes
  *****************************************************/
  registerEvaluacionCortesPrecision(user_to_register, token) {
    const json = JSON.stringify(user_to_register);
    const params = 'json=' + json + '&authorization=' + token;
    // console.log(json);
    const headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });

    return this._http.post(this.url + '/evaluaciones/new-evaluacion-cortes-precision', params, { headers: headers }).map(res => res.json());
  }
  ...
  * Fecha: 04-01-2019
  * Descripcion: Metodo Ajax, para Invocar el servicio
  * a la API ( /evaulaciones/list-evaluacion-corte-precision ).
  * Objetivo: Listar las Evaluaciones Practicas, por Alumno
  *****************************************************/
  getEvaluacionCortePrecisionByAlumno(alumno_to_find, token) {
    const json = JSON.stringify(alumno_to_find);
    const params = 'json=' + json + '&authorization=' + token;
    // console.log(json);
    const headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });

    return this._http.post(this.url + '/evaluaciones/list-evaluacion-corte-precision', params, { headers: headers }).map(res => res.json());
  }
