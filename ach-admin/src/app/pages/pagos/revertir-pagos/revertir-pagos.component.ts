...
import { RevertirPagosAlumnoModalComponent } from './revertir-pagos-alumno-modal.component';

// Datos que se van a Exportar a la Clase Hija
...
/**
 * @title Revertir Pagos de Alumno
 */
@Component({
  selector: 'app-revertir-pagos',
  templateUrl: './revertir-pagos.component.html',
  styleUrls: ['./revertir-pagos.component.scss'],
  providers: [ListasComunesService, RevertirPagosService, ConsultaAlumnoService],
})
export class RevertirPagosComponent implements OnInit {
  ...
  // Ventana Modal
  fileNameDialogRef: MatDialogRef<RevertirPagosAlumnoModalComponent>;
  ...
  openDialog(idAlumnoIn: number, nombresIn: string, apellidosIn: string,
    ...
    const dialogRef = this.dialog.open(RevertirPagosAlumnoModalComponent, {
      data: {
